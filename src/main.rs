...
use obielf::{
    ArtifactKind, Assembler, DefaultPolicyEngine, NASM_OBIELF32_FORMAT, NASM_OBIELF64_FORMAT,
    ObiElfFormat, PartFile, PartId, PartMetadata, TargetArtifact, TargetPackage, ValidationMetric,
    crc32,
};
...
fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        "formats" => {
            println!("{NASM_OBIELF32_FORMAT}");
            println!("{NASM_OBIELF64_FORMAT}");
            Ok(())
        }
        ...
        match arg.as_str() {
            ...
            "--kind" | "-k" => {
                ...
                kind = Some(ArtifactKind::from_str(&value).map_err(|error| {
                    format!(
                        "{error}; expected executable, object, static-library, dynamic-library, or metadata"
                    )
                })?);
            }
            "--name" | "-n" => {
...
fn usage() -> String {
    "usage:\n  cargo run -- formats\n  cargo run -- package --format obielf64 --kind executable --name NAME [--profile debug] [--target-dir target] INPUT..."
        .to_owned()
}

fn package_usage() -> String {
    "usage:\n  cargo run -- package --format <obielf32|obielf64> --kind <executable|object|static-library|dynamic-library|metadata> --name NAME [--profile debug] [--target-dir target] INPUT..."
        .to_owned()
}
